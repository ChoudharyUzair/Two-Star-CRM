...
const App = {
  ...
  // ===== Pagination =====
  // Each paginated list keeps its current page number here, keyed by a name.
  PAGE_SIZE: 20,
  _page: {},
  _getPage(key) { return this._page[key] || 1; },
  _setPage(key, p) { this._page[key] = Math.max(1, p); },
  // Change page for a list and re-run the given App method to re-render.
  goPage(key, p, rerenderMethod, ...args) {
    this._setPage(key, p);
    if (typeof this[rerenderMethod] === 'function') this[rerenderMethod](...args);
  },
  // Return { pageItems, totalPages, page, total, start } for an array.
  paginate(items, key) {
    const total = items.length;
    const size = this.PAGE_SIZE;
    const totalPages = Math.max(1, Math.ceil(total / size));
    let page = this._getPage(key);
    if (page > totalPages) { page = totalPages; this._setPage(key, page); }
    const start = (page - 1) * size;
    return { pageItems: items.slice(start, start + size), totalPages, page, total, start };
  },
  // Build a pagination control bar. rerenderMethod is called with args on click.
  renderPager(key, page, totalPages, total, rerenderMethod, ...args) {
    if (totalPages <= 1) {
      return total > 0
        ? `<div class="pager"><span class="pager-info">${total} ${total === 1 ? 'entry' : 'entries'}</span></div>`
        : '';
    }
    const argStr = args.map(a => (typeof a === 'string' ? `'${String(a).replace(/'/g, "\\'")}'` : a)).join(', ');
    const call = (p) => `App.goPage('${key}', ${p}, '${rerenderMethod}'${argStr ? ', ' + argStr : ''})`;
    const size = this.PAGE_SIZE;
    ...
    for (let p = 1; p <= totalPages; p++) {
      if (p === 1 || p === totalPages || (p >= page - win && p <= page + win)) nums.push(p);
      else if (nums[nums.length - 1] !== '...') nums.push('...');
    }
    const numHtml = nums.map(p => p === '...'
      ? `<span class="pager-ellipsis">…</span>`
      : `<button class="pager-btn ${p === page ? 'active' : ''}" onclick="${call(p)}">${p}</button>`).join('');
    return `
      <div class="pager">
        <span class="pager-info">Showing ${from}–${to} of ${total}</span>
        <div class="pager-controls">
          <button class="pager-btn" ${page <= 1 ? 'disabled' : ''} onclick="${call(page - 1)}"><i class="fas fa-chevron-left"></i></button>
          ${numHtml}
          <button class="pager-btn" ${page >= totalPages ? 'disabled' : ''} onclick="${call(page + 1)}"><i class="fas fa-chevron-right"></i></button>
        </div>
      </div>`;
  },
    ...
    document.getElementById('app').innerHTML = `
      ...
      <aside class="sidebar no-print" id="sidebar">
        ...
        <div class="nav-section">
          ...
          <button class="nav-btn ${this.state.view === 'banking' ? 'active' : ''}" id="nav-banking" onclick="App.showBanking()">
            <i class="fas fa-building-columns"></i><span>Banking / Payments</span>
          </button>
  ...
  async openClient(clientId) {
    ...
    this._ledgerSearch = '';   // reset the ledger entry search for the new client
    this.setActiveNav('');
    this.closeSidebarOnMobile();
    try {
      ...
      // Default to the LAST page so the newest transactions are shown first
      // (ledger is ordered oldest → newest).
      this._setPage('ledger', Math.max(1, Math.ceil(this.state.transactions.length / this.PAGE_SIZE)));
      this.renderFolders();
      this.renderLedger();
    ...
    area.innerHTML = `
      <div class="page-header">
        ...
        <div class="flex gap-2 flex-wrap">
          ${isSupplier
            ? `<button onclick="App.showSupplierRates()" class="btn btn-primary btn-sm" title="Is supplier se items kis rate par lete hain — buy rate set karein"><i class="fas fa-tags"></i> Product Rates</button>`
            : `<button onclick="App.showCustomerRates()" class="btn btn-primary btn-sm" title="Is customer ke liye products ke special selling rate set karein"><i class="fas fa-tags"></i> Product Rates</button>`}
          <button onclick="App.showLedgerPdfModal()" class="btn btn-success btn-sm" title="Customer ko ledger PDF form mein bhejein"><i class="fas fa-file-pdf"></i> Send PDF</button>
      ...
      <div class="p-4 md:p-6 space-y-5">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-3 border-b flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              <div class="relative">
                ...
                <input type="text" id="ledger-search" placeholder="Search entries (date, bill, amount, note)..."
                  class="input-field pl-8" style="max-width:280px;padding-top:6px;padding-bottom:6px;"
                  value="${this.escapeAttr(this._ledgerSearch || '')}"
                  oninput="App.onLedgerSearch(this.value)">
              </div>
          ...
          <div id="ledger-pager">${this._renderLedgerPager()}</div>
        </div>
      </div>`;
  },

  _renderLedgerPager() {
    // While a search is active we show all matches on one page, so hide the pager.
    if ((this._ledgerSearch || '').trim()) return '';
    const total = this.state.transactions.length;
    const { totalPages, page } = this.paginate(this.state.transactions, 'ledger');
    return this.renderPager('ledger', page, totalPages, total, 'refreshLedgerBody');
  },

  // Re-render just the ledger table body + pager when the page changes.
  refreshLedgerBody() {
    const opening = parseFloat((this.state.currentClient || {}).opening_balance) || 0;
    const body = document.getElementById('ledger-body');
    const pager = document.getElementById('ledger-pager');
    if (body) body.innerHTML = this.renderRows(opening);
    if (pager) pager.innerHTML = this._renderLedgerPager();
    try { document.getElementById('ledger-table').scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) {}
  },

  // Does a ledger transaction match the current search term? Searches across
  // date, bill no, amounts, status, description and any custom-column values.
  _ledgerRowMatches(t, term) {
    if (!term) return true;
    const q = term.toLowerCase();
    const customData = (() => { try { return JSON.parse(t.custom_data || '{}'); } catch { return {}; } })();
    const parts = [
      t.entry_date, t.bill_no, t.description, t.status,
      String(parseFloat(t.amount_pending) || 0),
      String(parseFloat(t.amount_received) || 0),
  ...
  onLedgerSearch(val) {
    this._ledgerSearch = val || '';
    // Reset to first page whenever the filter changes so results are visible.
    this._setPage('ledger', 1);
    this.refreshLedgerBody();
  ...
  renderRows(opening) {
    ...
    const term = (this._ledgerSearch || '').trim();
    // When searching, show ALL matching rows (ignore pagination) but still
    // compute the running balance from the full, unfiltered list so every row's
    // balance stays correct. When not searching, paginate as before.
    const { page } = this.paginate(this.state.transactions, 'ledger');
    const startIdx = term ? 0 : (page - 1) * this.PAGE_SIZE;
    const endIdx = term ? this.state.transactions.length : startIdx + this.PAGE_SIZE;
    let running = opening;
    let shown = 0;
    const html = this.state.transactions.map((t, i) => {
      ...
      if (term && !this._ledgerRowMatches(t, term)) return '';
      shown++;
      const customData = (() => { try { return JSON.parse(t.custom_data || '{}'); } catch { return {}; } })();
    ...
    if (term && shown === 0) {
      return `<tr><td colspan="${9 + this.state.customColumns.length}" class="text-center py-8 text-gray-500">
        <i class="fas fa-magnifying-glass text-3xl mb-2 block"></i>"${this.escapeHtml(term)}" ke liye koi entry nahi mili.</td></tr>`;
    }
    return html;
  },

  // === EDIT LEDGER ROW (POPUP, BILL-STYLE) ===
  async editLedgerRow(id) {
    const t = this.state.transactions.find(t => t.id === id);
    if (!t) return;
    await this.loadBankAccounts();
    // Is this ledger a supplier (money OUT) or customer (money IN) context?
    ...
    this.openModal(`
      ...
      <form id="ledger-edit-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>${this.bankSelectHtml('er-bank', t.bank_account_id || '', true, (isSup ? 'Pay from Bank/Cash' : 'Receive into Bank/Cash'))}
          <p class="text-xs text-gray-500 mt-1"><i class="fas fa-info-circle mr-1"></i>${isSup ? 'Amount is subtracted from the selected account.' : 'Amount received is added to the selected account.'}</p></div>
      ...
      try {
        const res = await fetch(`/api/transactions/${id}`, {
          ...
          body: JSON.stringify({
            ...
            bank_account_id: document.getElementById('er-bank').value || null,
  ...
  // ===== LEDGER PDF (Feature #7) =====
  // Show a small modal to choose how many recent entries to include, then
  // generate a Two Star branded ledger PDF for the current customer.
  showLedgerPdfModal() {
    const c = this.state.currentClient;
    if (!c) return;
    const totalRows = this.state.transactions.length;
    this.openModal(`
      <h2 class="text-xl font-bold mb-1"><i class="fas fa-file-pdf text-red-500 mr-2"></i>Ledger PDF — ${this.escapeHtml(c.name)}</h2>
      <p class="text-sm text-gray-500 mb-4">Customer ko uska khata (ledger) PDF form mein bhejein. Logo automatically PDF par lagega.</p>
      <div class="space-y-3">
        <div>
          <label class="block text-sm font-medium mb-1">Kitni aakhri (last) entries ka PDF banana hai?</label>
          <select id="lpdf-count" class="input-field">
            ...
            <option value="all">All entries (${totalRows})</option>
          </select>
          <p class="text-xs text-gray-400 mt-1">Total ${totalRows} entries maujood hain. Running balance poori history par calculate hota hai.</p>
        </div>
      </div>
      <div class="flex gap-2 justify-end pt-4 mt-3 border-t flex-wrap">
        <button class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
        <button class="btn btn-primary" onclick="App.downloadLedgerPDF()"><i class="fas fa-download"></i> Download PDF</button>
        <button class="btn btn-success" onclick="App.shareLedgerPDF()"><i class="fab fa-whatsapp"></i> WhatsApp (PDF)</button>
      </div>`);
  },

  _selectedLedgerCount() {
    const sel = document.getElementById('lpdf-count');
    const v = sel ? sel.value : '50';
    return v === 'all' ? Infinity : (parseInt(v) || 50);
  },

  // Build an off-screen branded ledger HTML node and rasterize it into a jsPDF.
  // Returns { blob, fileName } or null.
  async _buildLedgerPDF(limitCount) {
    const c = this.state.currentClient;
    if (!c) return null;
    ...
    // Compute running balance across ALL transactions (chronological), then take
    // only the last `limitCount` rows for display.
    const txAll = this.state.transactions.slice();
    let running = opening;
    const withRunning = txAll.map(t => {
      const rec = parseFloat(t.amount_received) || 0;
      const pen = parseFloat(t.amount_pending) || 0;
      running = running + pen - rec;
      return { t, rec, pen, running };
    });
    const rowsToShow = (limitCount === Infinity) ? withRunning : withRunning.slice(-limitCount);
    let totalReceived = 0, totalPending = 0;
    txAll.forEach(t => { totalReceived += parseFloat(t.amount_received) || 0; totalPending += parseFloat(t.amount_pending) || 0; });
    const netBalance = opening + totalPending - totalReceived;

    // Logo (preserve aspect ratio like the invoice does)
    ...
    if (hasLogo) {
      ...
      if (dim && dim.w > 0 && dim.h > 0) {
        ...
        w = Math.round(dim.w * scale); h = Math.round(dim.h * scale);
      }
      const sizeAttrs = (w && h)
        ? `width="${w}" height="${h}" style="width:${w}px;height:${h}px;object-fit:contain;display:block;"`
        : `style="max-width:${MAX_W}px;max-height:${MAX_H}px;object-fit:contain;display:block;"`;
      logoHtml = `<img src="${this.escapeAttr(b.logo_url)}" alt="logo" ${sizeAttrs}>`;
    } else {
      logoHtml = `<div style="font-size:28px;font-weight:800;color:#4f46e5;">${this.escapeHtml((b.company_name || 'Two Star').split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase())}</div>`;
    }

    const genDate = new Date().toISOString().slice(0, 10);
    // Shared cell style: proper padding + row divider so columns never overlap
    // and every row keeps its own height (fixes the last row being clipped by TOTALS).
    const cellBase = 'padding:8px 10px;border-bottom:1px solid #e2e8f0;vertical-align:top;';
    const rowsHtml = rowsToShow.map((r, i) => `
      <tr style="background:${i % 2 === 0 ? '#ffffff' : '#f8fafc'};">
        <td style="${cellBase}text-align:center;color:#64748b;white-space:nowrap;">${i + 1}</td>
        <td style="${cellBase}white-space:nowrap;">${this.escapeHtml(r.t.entry_date || '')}</td>
        <td style="${cellBase}white-space:nowrap;">${this.escapeHtml(r.t.bill_no || '')}</td>
        <td style="${cellBase}text-align:right;white-space:nowrap;">${r.pen > 0 ? this.fmt(r.pen) : '—'}</td>
        <td style="${cellBase}text-align:right;white-space:nowrap;">${this.fmt(r.rec)}</td>
        <td style="${cellBase}word-break:break-word;">${this.escapeHtml(r.t.description || '')}</td>
        <td style="${cellBase}text-align:right;font-weight:700;white-space:nowrap;">${this.fmt(r.running)}</td>
      </tr>`).join('');
    ...
    wrap.innerHTML = `
      <div id="ledger-pdf-area" style="width:800px;padding:28px;font-family:Arial,Helvetica,sans-serif;color:#1e293b;box-sizing:border-box;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #4f46e5;padding-bottom:14px;margin-bottom:14px;">
          <div>${logoHtml}
            ${hasLogo ? '' : `<div style="font-size:16px;font-weight:700;margin-top:4px;">${this.escapeHtml(b.company_name || 'Two Star Industries')}</div>`}
            <div style="font-size:11px;color:#475569;margin-top:6px;line-height:1.5;">
              ${b.bill_address ? `<div>${this.escapeHtml(b.bill_address)}</div>` : ''}
              ${b.bill_phone ? `<div>Phone: ${this.escapeHtml(b.bill_phone)}</div>` : ''}
              ${b.bill_email ? `<div>${this.escapeHtml(b.bill_email)}</div>` : ''}
            </div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:24px;font-weight:800;color:#4f46e5;letter-spacing:1px;">LEDGER</div>
            <div style="font-size:11px;color:#64748b;margin-top:6px;">Date: ${genDate}</div>
          </div>
        </div>

        <div style="display:flex;justify-content:space-between;gap:16px;margin-bottom:14px;">
          <div style="background:#f8fafc;border-radius:8px;padding:10px 14px;flex:1;">
            <div style="font-size:10px;color:#64748b;text-transform:uppercase;">${isSupplier ? 'Supplier' : 'Customer'}</div>
            <div style="font-size:15px;font-weight:700;">${this.escapeHtml(c.name)}</div>
            ${c.phone ? `<div style="font-size:11px;color:#475569;margin-top:2px;">${this.escapeHtml(c.phone)}</div>` : ''}
            ${c.address ? `<div style="font-size:11px;color:#475569;">${this.escapeHtml(c.address)}</div>` : ''}
          </div>
          <div style="background:#eef2ff;border-radius:8px;padding:12px 16px;text-align:right;min-width:210px;">
            <div style="font-size:10px;color:#64748b;line-height:1.6;">Opening Balance: PKR ${this.fmt(opening)}</div>
            <div style="font-size:10px;color:#16a34a;line-height:1.6;">Total Received: PKR ${this.fmt(totalReceived)}</div>
            <div style="font-size:16px;font-weight:800;color:#4f46e5;margin-top:8px;padding-top:8px;border-top:1px solid #c7d2fe;">${isSupplier ? 'Outstanding' : 'Balance'}: PKR ${this.fmt(netBalance)}</div>
          </div>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;table-layout:fixed;font-size:11px;">
          <thead>
            <tr style="background:#4f46e5;color:#fff;">
              <th style="padding:9px 10px;text-align:center;width:34px;">#</th>
              <th style="padding:9px 10px;text-align:left;width:82px;">Date</th>
              <th style="padding:9px 10px;text-align:left;width:72px;">Bill No</th>
              <th style="padding:9px 10px;text-align:right;width:84px;">Pending</th>
              <th style="padding:9px 10px;text-align:right;width:84px;">Received</th>
              <th style="padding:9px 10px;text-align:left;">Description</th>
              <th style="padding:9px 10px;text-align:right;width:96px;">Balance</th>
            </tr>
          </thead>
          <tbody>
            ${rowsToShow.length === 0 ? '<tr><td colspan="7" style="text-align:center;padding:20px;color:#94a3b8;">No entries</td></tr>' : rowsHtml}
          </tbody>
          <tfoot>
            <tr style="background:#f1f5f9;font-weight:700;border-top:2px solid #4f46e5;">
              <td colspan="4" style="padding:10px;text-align:right;border-top:2px solid #4f46e5;">TOTALS:</td>
              <td style="padding:10px;text-align:right;border-top:2px solid #4f46e5;white-space:nowrap;">${this.fmt(totalReceived)}</td>
              <td style="border-top:2px solid #4f46e5;"></td>
              <td style="padding:10px;text-align:right;color:#4f46e5;border-top:2px solid #4f46e5;white-space:nowrap;">PKR ${this.fmt(netBalance)}</td>
            </tr>
          </tfoot>
        </table>

        <div style="margin-top:22px;padding-top:12px;border-top:1px solid #e2e8f0;text-align:center;font-size:10px;color:#94a3b8;">
          ${this.escapeHtml(b.bill_footer || 'Thank you for your business!')}
        </div>
      </div>`;
    document.body.appendChild(wrap);
    const node = wrap.querySelector('#ledger-pdf-area');
    ...
    try {
      ...
      const safeName = String(c.name || 'Customer').replace(/[^a-zA-Z0-9_-]/g, '_');
      result = { blob: pdf.output('blob'), fileName: `Ledger-${safeName}.pdf` };
    } catch (e) {
      this.toast('PDF banane mein error', 'error');
    } finally {
      wrap.remove();
    }
    return result;
  ...
  async downloadLedgerPDF() {
    const count = this._selectedLedgerCount();
    this.toast('PDF ban raha hai...', 'info');
    const res = await this._buildLedgerPDF(count);
    if (!res) return;
  ...
  async shareLedgerPDF() {
    ...
    const opening = parseFloat(c.opening_balance) || 0;
    let totalReceived = 0, totalPending = 0;
    this.state.transactions.forEach(t => { totalReceived += parseFloat(t.amount_received) || 0; totalPending += parseFloat(t.amount_pending) || 0; });
    const netBalance = opening + totalPending - totalReceived;
    const caption = [
      `*${b.company_name || 'Two Star Industries'}* — Ledger`,
      `${c.name}`,
      `Balance: PKR ${this.fmt(netBalance)}`,
      ``,
      `Ledger PDF attached.`
    ].join('\n');
    ...
    // Fallback: download the PDF and open WhatsApp chat with a caption.
    const url = URL.createObjectURL(res.blob);
    const a = document.createElement('a');
    a.href = url; a.download = res.fileName;
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 4000);
    const waNum = this._waNumber(c.phone);
    const waUrl = waNum
      ? `https://wa.me/${waNum}?text=${encodeURIComponent(caption)}`
      : `https://wa.me/?text=${encodeURIComponent(caption)}`;
    window.open(waUrl, '_blank');
    this.toast('PDF download ho gaya — WhatsApp par attach kar dein', 'info');
    this.closeModal();
  },
      ...
      : this._custRates.map(rt => `
        <tr class="border-t">
          ...
          <td class="p-2 text-center whitespace-nowrap">
            <button onclick="App._editCustomerRate(${rt.inventory_id})" class="text-blue-500 hover:text-blue-700 mr-2" title="Edit rate"><i class="fas fa-edit text-sm"></i></button>
            <button onclick="App._deleteCustomerRate(${rt.id})" class="text-red-500 hover:text-red-700" title="Delete rate"><i class="fas fa-trash text-sm"></i></button>
          </td>
        </tr>`).join('');
  ...
  // Edit an existing customer product rate: pre-fill the form with the product
  // and its current special rate so the user can update it in place.
  _editCustomerRate(inventoryId) {
    const rt = (this._custRates || []).find(r => String(r.inventory_id) === String(inventoryId));
    if (!rt) return;
    const sel = document.getElementById('cr-product');
    const box = document.getElementById('cr-rate');
    if (sel) sel.value = String(inventoryId);
    if (box) { box.value = rt.rate || 0; box.focus(); box.select(); }
    // Scroll the add/update panel into view for convenience on mobile
    try { sel && sel.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (e) {}
    this.toast('Rate box mein value aa gayi — update karke "Add / Update Rate" dabayen', 'info');
  },
  ...
  // ===== #4 + #5: Per-supplier BUY rate list (mirrors Customer Product Rates) =====
  // The supplier profile gets a "Product Rates" list where we record the rate we
  // PAY this supplier for each item. It covers BOTH inventory products and raw
  // materials (one shared list). These rates auto-fill when restocking from this
  // supplier — so restock uses the buy rate, never the selling rate.
  async showSupplierRates() {
    const c = this.state.currentClient;
    if (!c) return;
    // Load inventory + raw materials for the item picker
    if (!this.state.inventory || this.state.inventory.length === 0) {
      try { const inv = await this.api.get('/api/inventory'); this.state.inventory = inv.inventory || inv.items || []; } catch (e) {}
    }
    let rawItems = [];
    try { const rm = await this.api.get('/api/raw-materials'); rawItems = rm.items || []; } catch (e) {}
    this._supRawItems = rawItems;
    let rates = [];
    try { const r = await this.api.get(`/api/clients/${c.id}/supplier-rates`); rates = r.rates || []; } catch (e) {}
    this._supRates = rates;
    const products = this.state.inventory || [];

    const renderRows = () => (this._supRates.length === 0)
      ? '<tr><td colspan="5" class="text-center text-gray-500 py-4">Abhi koi buy-rate set nahi. Neeche se add karein.</td></tr>'
      : this._supRates.map(rt => `
        <tr class="border-t">
          <td class="p-2"><strong>${this.escapeHtml(rt.live_name || rt.item_name || '-')}</strong>
            ${rt.item_unit ? `<span class="text-xs text-gray-400 ml-1">${this.escapeHtml(rt.item_unit)}</span>` : ''}</td>
          <td class="p-2"><span class="px-2 py-0.5 rounded-full text-xs ${rt.item_type === 'raw' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}">${rt.item_type === 'raw' ? 'Raw Material' : 'Inventory'}</span></td>
          <td class="p-2 text-right text-gray-500">PKR ${this.fmt(rt.current_rate || 0)}</td>
          <td class="p-2 text-right font-bold text-green-700">PKR ${this.fmt(rt.rate || 0)}</td>
          <td class="p-2 text-center whitespace-nowrap">
            <button onclick="App._editSupplierRate('${rt.item_type}', ${rt.item_id})" class="text-blue-500 hover:text-blue-700 mr-2" title="Edit rate"><i class="fas fa-edit text-sm"></i></button>
            <button onclick="App._deleteSupplierRate(${rt.id})" class="text-red-500 hover:text-red-700" title="Delete rate"><i class="fas fa-trash text-sm"></i></button>
          </td>
        </tr>`).join('');

    this.openModal(`
      <h2 class="text-xl font-bold mb-1"><i class="fas fa-tags text-amber-600 mr-2"></i>Supplier Buy Rates — ${this.escapeHtml(c.name)}</h2>
      <p class="text-sm text-gray-500 mb-3"><i class="fas fa-info-circle mr-1"></i>Yahan is supplier se har item ka <b>buy rate</b> (jis rate par hum lete hain) set karein. Jab is supplier se restock karenge (Inventory ya Raw Material), ye rate <b>khud-ba-khud</b> lag jayega — selling rate nahi.</p>

      <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-2 items-end">
          <div>
            <label class="block text-xs font-medium mb-1">Type</label>
            <select id="sr-type" class="input-field" onchange="App._srTypeChange()">
          ...
          <div>
            ...
            <select id="sr-item" class="input-field"></select>
          </div>
          <div>
            <label class="block text-xs font-medium mb-1">Buy Rate (PKR)</label>
            <input id="sr-rate" type="number" step="any" min="0" class="input-field" placeholder="Is supplier ka rate">
          </div>
          <div>
            <button onclick="App._saveSupplierRate()" class="btn btn-primary w-full"><i class="fas fa-plus"></i> Add / Update</button>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border rounded-lg overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-50"><tr>
            ...
            <th class="text-right p-2">Current Rate</th>
            <th class="text-right p-2">Supplier Buy Rate</th>
            <th class="text-center p-2"></th>
          </tr></thead>
          <tbody id="sr-rows">${renderRows()}</tbody>
        </table>
      </div>
      <div class="flex justify-end pt-3"><button class="btn btn-secondary" onclick="App.closeModal()">Close</button></div>
    `, 'modal-lg');
    ...
    if (sel) sel.addEventListener('change', () => {
      ...
      const existing = this._supRates.find(r => r.item_type === type && String(r.item_id) === sel.value);
      const opt = sel.options[sel.selectedIndex];
      const box = document.getElementById('sr-rate');
      if (box) box.value = existing ? (existing.rate || 0) : (opt?.dataset.rate || 0);
    });
  },

  // Populate the item dropdown based on the chosen type (inventory vs raw).
  _srTypeChange() {
    const type = document.getElementById('sr-type')?.value || 'inventory';
    const sel = document.getElementById('sr-item');
    if (!sel) return;
    const list = type === 'raw' ? (this._supRawItems || []) : (this.state.inventory || []);
    sel.innerHTML = '<option value="">-- Select item --</option>' +
      list.map(p => `<option value="${p.id}" data-rate="${p.rate || 0}">${this.escapeHtml(p.name)} (rate PKR ${this.fmt(p.rate || 0)})</option>`).join('');
  },

  async _saveSupplierRate() {
    const c = this.state.currentClient;
    const type = document.getElementById('sr-type')?.value || 'inventory';
    const itemId = document.getElementById('sr-item')?.value;
    const rate = parseFloat(document.getElementById('sr-rate')?.value);
    if (!itemId) { this.toast('Pehle item select karein', 'error'); return; }
    if (isNaN(rate)) { this.toast('Rate likhein', 'error'); return; }
    try {
      await this.api.post(`/api/clients/${c.id}/supplier-rates`, { item_type: type, item_id: parseInt(itemId), rate });
      this.toast('Buy rate saved', 'success');
      this.showSupplierRates();
    } catch (e) { this.toast('Save failed', 'error'); }
  },

  _editSupplierRate(itemType, itemId) {
    const rt = (this._supRates || []).find(r => r.item_type === itemType && String(r.item_id) === String(itemId));
    if (!rt) return;
    const typeSel = document.getElementById('sr-type');
    if (typeSel) { typeSel.value = itemType; this._srTypeChange(); }
    const sel = document.getElementById('sr-item');
    const box = document.getElementById('sr-rate');
    if (sel) sel.value = String(itemId);
    if (box) { box.value = rt.rate || 0; box.focus(); box.select(); }
    try { typeSel && typeSel.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (e) {}
    this.toast('Rate box mein value aa gayi — update karke "Add / Update" dabayen', 'info');
  },

  async _deleteSupplierRate(rid) {
    const c = this.state.currentClient;
    if (!confirm('Is buy-rate ko hata dein?')) return;
    try {
      await this.api.delete(`/api/clients/${c.id}/supplier-rates/${rid}`);
      this.toast('Rate removed', 'success');
      this.showSupplierRates();
    } catch (e) { this.toast('Failed', 'error'); }
  },
  ...
  async showDashboard() {
    ...
    try {
      const [data, pData, bankData] = await Promise.all([
        this.api.get('/api/dashboard'),
        this.api.get('/api/products'),
        this.api.get('/api/banking/summary').catch(() => null)
      ]);
      this.state.products = pData.products || [];
      data._banking = bankData || null;
      this.renderDashboard(data);
    } catch (e) {}
  },

  // Banking summary strip for the dashboard (total balance + per-account cards).
  renderBankingDashSection(bank) {
    if (!bank) return '';
    const accs = bank.accounts || [];
    const totalBal = parseFloat(bank.total_balance) || 0;
    const totalIn = parseFloat(bank.total_in) || 0;
    const totalOut = parseFloat(bank.total_out) || 0;
    const cards = accs.length === 0
      ? `<div class="col-span-full text-center py-4 text-gray-400 text-sm"><i class="fas fa-building-columns mr-1"></i>No bank/cash accounts yet. <button onclick="App.showBanking()" class="text-blue-600 underline">Add one</button> to start tracking money in & out.</div>`
      : accs.map(a => {
          const m = this.acctTypeMeta(a.account_type);
          const bal = parseFloat(a.current_balance) || 0;
          return `<button onclick="App.showBankAccount(${a.id})" class="stat-card text-left hover:shadow-md transition" style="border-left:4px solid ${a.color || '#3b82f6'}">
            <p class="text-xs text-gray-500"><i class="fas ${a.icon || m.icon} mr-1"></i>${this.escapeHtml(a.name)}</p>
            <p class="text-lg font-bold mt-1 ${bal < 0 ? 'text-red-600' : 'text-gray-800'}">PKR ${this.fmt(bal)}</p>
          </button>`;
        }).join('');
    return `
      <section id="dash-banking">
        <h2 class="dash-section-title"><i class="fas fa-building-columns text-blue-600"></i>Banking / Payments
          <button onclick="App.showBanking()" class="text-xs font-normal text-blue-600 underline ml-2">Open</button></h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
          <div class="stat-card" style="background:linear-gradient(135deg,#1e3a8a,#3b82f6);color:#fff">
            <p class="text-xs opacity-90"><i class="fas fa-wallet mr-1"></i>Total Balance (all accounts)</p>
            <p class="text-2xl font-bold mt-1">PKR ${this.fmt(totalBal)}</p>
            <p class="text-xs opacity-80 mt-1">${accs.length} account(s)</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-arrow-down text-green-500 mr-1"></i>Total Money In</p>
            <p class="text-xl font-bold mt-1 amount-received">PKR ${this.fmt(totalIn)}</p><p class="text-xs text-gray-400 mt-1">excl. transfers</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-arrow-up text-red-500 mr-1"></i>Total Money Out</p>
            <p class="text-xl font-bold mt-1 amount-pending">PKR ${this.fmt(totalOut)}</p><p class="text-xs text-gray-400 mt-1">excl. transfers</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-scale-balanced mr-1"></i>Net Flow</p>
            <p class="text-xl font-bold mt-1 ${(totalIn-totalOut)<0?'text-red-600':'text-green-600'}">PKR ${this.fmt(totalIn - totalOut)}</p></div>
        </div>
  ...
  renderDashboard(data) {
    ...
    area.innerHTML = `
      ...
      <div class="p-4 md:p-6 space-y-6">
        ...
        ${this.renderBankingDashSection(data._banking)}
  ...
  async showInventory() {
    ...
    try {
      const [data, mov] = await Promise.all([
        ...
        this.api.get('/api/inventory/movements?limit=1000')
      ]);
      this.state.inventory = data.items || [];
      this.state.invMovements = (mov && mov.movements) || [];
      this._setPage('invRecent', 1);
      ...
      <div class="p-4 md:p-6 space-y-5">
        ...
        <section id="inv-recent" class="bg-white rounded-xl shadow-sm p-5">
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-xs text-gray-500">Stock movements (sold / returned / packed / adjusted)</span>
          </div>
          <div id="inv-recent-body">${this.renderInventoryMovements()}</div>
        </section>
      </div>`;
  },
  ...
  renderInventoryMovements() {
    ...
    const badge = (t) => {
      ...
      if (t === 'return')  return '<span class="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700"><i class="fas fa-arrow-down mr-1"></i>Cust. Return</span>';
      if (t === 'supplier_return') return '<span class="px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-700"><i class="fas fa-rotate-left mr-1"></i>Sup. Return</span>';
      ...
      if (t === 'production') return '<span class="px-2 py-0.5 rounded-full text-xs bg-purple-100 text-purple-700"><i class="fas fa-industry mr-1"></i>Packed</span>';
      return '<span class="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700"><i class="fas fa-sliders mr-1"></i>Adjusted</span>';
    };
    const { pageItems, totalPages, page, total } = this.paginate(movs, 'invRecent');
    const rows = pageItems.map(m => {
      ...
      const isAuto = m.type === 'production';
      return `<tr>
        <td class="text-gray-600">${this.escapeHtml(m.entry_date || '')}</td>
        <td class="font-medium">${this.escapeHtml(m.item_name || m.product_name || '—')}</td>
        <td>${badge(m.type)}</td>
        <td class="text-right ${(m.type === 'sale' || m.type === 'supplier_return') ? 'text-red-600' : (m.type === 'return' || m.type === 'restock' || m.type === 'production') ? 'text-green-600' : 'text-gray-700'} font-semibold">${(m.type === 'sale' || m.type === 'supplier_return') ? '-' : (m.type === 'return' || m.type === 'restock' || m.type === 'production') ? '+' : ''}${this.fmt(qty)}</td>
        ...
        <td class="text-center whitespace-nowrap">
          ${isAuto ? '<span class="text-gray-300" title="Packed-from-manufacturing — edit Products Manufacturing se">—</span>' : `<button onclick="App.editMovement(${m.id})" class="text-blue-500 hover:text-blue-700 mr-2" title="Edit"><i class="fas fa-edit text-sm"></i></button>`}
          <button onclick="App.deleteMovement(${m.id})" class="text-red-500 hover:text-red-700" title="Delete & reverse stock"><i class="fas fa-trash text-sm"></i></button>
        </td>
      </tr>`;
    }).join('');
    return `
      <div class="overflow-x-auto"><table class="ledger-table">
        <thead><tr>
          ...
          <th>Customer / Note</th><th style="width:100px;" class="text-center">Action</th>
        </tr></thead><tbody>${rows}</tbody></table></div>
      ${this.renderPager('invRecent', page, totalPages, total, 'refreshInvRecent')}`;
  },

  refreshInvRecent() {
    const el = document.getElementById('inv-recent-body');
    if (el) el.innerHTML = this.renderInventoryMovements();
    try { document.getElementById('inv-recent').scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) {}
  },

  // Edit a manual inventory movement (sale / return / restock / adjust).
  editMovement(id) {
    const m = (this.state.invMovements || []).find(x => x.id === id);
    if (!m) return;
    if (m.type === 'production') { this.toast('Packed entries ki edit Products Manufacturing se karein', 'error'); return; }
    if (m.type === 'return' || m.type === 'supplier_return') {
      // These carry a linked ledger transaction — editing quantity/rate here would
      // desync the ledger. Ask the user to delete & re-add instead (delete also
      // removes the linked ledger row).
      this.toast('Return entry ko edit karne ke liye ise delete karke dobara add karein (ledger auto-adjust ho jayega).', 'info');
      return;
    }
    const today = new Date().toISOString().slice(0, 10);
    // BUG1 FIX: prefer the stored direction (+1/-1) over guessing from notes text
    const dirGuess = (m.direction === -1) ? 'out' : (m.direction === 1 ? 'in' : ((m.notes && /out/i.test(m.notes)) ? 'out' : 'in'));
    this.openModal(`
      <h2 class="text-xl font-bold mb-3"><i class="fas fa-edit text-blue-500 mr-2"></i>Edit Stock Movement</h2>
      <p class="text-sm text-gray-500 mb-3">Product: <strong>${this.escapeHtml(m.item_name || m.product_name || '')}</strong></p>
      <form id="mov-edit-form" class="space-y-3">
        <div><label class="block text-sm font-medium mb-1">Type *</label>
          <select id="me-type" class="input-field" onchange="App._meToggleDir()">
            <option value="sale" ${m.type === 'sale' ? 'selected' : ''}>Sold (stock decreases)</option>
            <option value="restock" ${m.type === 'restock' ? 'selected' : ''}>Restock (stock increases)</option>
            <option value="adjust" ${m.type === 'adjust' ? 'selected' : ''}>Adjustment</option>
          </select>
          <p class="text-xs text-gray-500 mt-1"><i class="fas fa-info-circle mr-1"></i>Customer/Supplier return ki entry yahan se edit nahi hoti — delete karke dobara add karein (ledger auto-adjust).</p></div>
        <div id="me-dir-wrap" style="display:${m.type === 'adjust' ? 'block' : 'none'};"><label class="block text-sm font-medium mb-1">Adjust Direction</label>
          <select id="me-dir" class="input-field">
            <option value="in" ${dirGuess === 'in' ? 'selected' : ''}>Add to stock (+)</option>
            <option value="out" ${dirGuess === 'out' ? 'selected' : ''}>Remove from stock (−)</option>
          </select></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm font-medium mb-1">Quantity *</label><input id="me-qty" type="number" step="any" min="0" class="input-field" value="${parseFloat(m.quantity) || 0}"></div>
          <div><label class="block text-sm font-medium mb-1">Rate (PKR)</label><input id="me-rate" type="number" step="any" class="input-field" value="${parseFloat(m.rate) || 0}"></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm font-medium mb-1">Date</label><input id="me-date" type="date" class="input-field" value="${m.entry_date || today}"></div>
          <div><label class="block text-sm font-medium mb-1">Customer</label><input id="me-cust" type="text" class="input-field" value="${this.escapeAttr(m.customer_name || '')}"></div>
        </div>
        <div><label class="block text-sm font-medium mb-1">Note</label><input id="me-notes" type="text" class="input-field" value="${this.escapeAttr(m.notes || '')}"></div>
        <div class="flex gap-2 justify-end pt-2 border-t">
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save</button>
        </div>
      </form>`);
    document.getElementById('mov-edit-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const payload = {
        type: document.getElementById('me-type').value,
        direction: document.getElementById('me-dir').value,
        quantity: parseFloat(document.getElementById('me-qty').value) || 0,
        rate: parseFloat(document.getElementById('me-rate').value) || 0,
        entry_date: document.getElementById('me-date').value,
        customer_name: document.getElementById('me-cust').value,
        notes: document.getElementById('me-notes').value
      };
      if (payload.quantity <= 0) { this.toast('Quantity must be > 0', 'error'); return; }
      try {
        const res = await this.api.put(`/api/inventory/movements/${id}`, payload);
        if (res && res.error) { this.toast(res.error, 'error'); return; }
        this.closeModal();
        await this.showInventory();
        this.toast('Movement updated', 'success');
      } catch (err) { this.toast('Update failed', 'error'); }
    });
  },
  ...
  // ============ UNIFIED Record Stock Movement (Tasks 2, 3, 5) ============
  // ONE modal handles every stock change so nothing is duplicated:
  //   • Sold            → stock down (optional customer name)
  //   • Customer Return → stock up  + customer ledger CREDIT   (was the old
  //                       separate "Customer Return" button — now merged here)
  //   • Supplier Return → stock down + supplier ledger credit-note (money we
  //                       no longer owe)  [NEW — request #3]
  //   • Restock         → stock up  + supplier ledger DEBIT; rate auto-fills
  //                       from the SUPPLIER's buy-rate list, not the selling
  //                       rate  [bug fix — request #5]
  //   • Adjustment      → manual in/out
  showMovementModal() {
    const items = this.state.inventory || [];
    if (items.length === 0) { this.toast('Add a product first', 'error'); return; }
    const today = new Date().toISOString().slice(0, 10);
    this._movSupRateMap = {};   // { "inventory:<id>": buyRate } for the selected supplier
    this.openModal(`
      ...
      <form id="mov-form" class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div><label id="m-rate-label" class="block text-sm font-medium mb-1">Rate / Cost (PKR)</label><input id="m-rate" type="number" step="any" class="input-field" placeholder="Rate"></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
    ...
    document.getElementById('mov-form').addEventListener('submit', async (e) => {
      ...
      const inventory_id = parseInt(document.getElementById('m-item').value);
      ...
      try {
        ...
        const payload = {
          inventory_id, type: typeVal,
          direction: document.getElementById('m-dir').value,
          quantity, rate, entry_date,
  ...
  async _movSupplierChange() {
    ...
    if (sel && manualWrap) manualWrap.style.display = sel.value === '__manual__' ? 'block' : 'none';
    this._movSupRateMap = {};
    const rateHint = document.getElementById('m-supplier-rate-hint');
    const id = sel ? parseInt(sel.value) : 0;
    if (id && sel.value !== '__manual__') {
      try {
        const data = await this.api.get(`/api/clients/${id}/supplier-rate-map`);
        this._movSupRateMap = data.rateMap || {};
      } catch (e) { this._movSupRateMap = {}; }
    }
    this._movApplySupplierRate();
    if (rateHint) {
      const n = Object.keys(this._movSupRateMap).length;
      rateHint.style.display = (id && sel.value !== '__manual__') ? 'block' : 'none';
      const span = rateHint.querySelector('span');
      if (span) span.textContent = n > 0
        ? `Is supplier ke ${n} item ke buy-rate save hain — select product ka rate khud lag jayega. (edit bhi kar sakte hain)`
        : `Is supplier ke koi saved buy-rate nahi — rate manually likhein. Supplier profile > Product Rates se save kar sakte hain.`;
    }
  },

  // Fill the rate box from the supplier buy-rate map for the current product.
  _movApplySupplierRate() {
    const itemSel = document.getElementById('m-item');
    const rateEl = document.getElementById('m-rate');
    if (!itemSel || !rateEl) return;
    const key = 'inventory:' + itemSel.value;
    if (this._movSupRateMap && this._movSupRateMap[key] != null) {
      rateEl.value = this._movSupRateMap[key];
    }
  },
  ...
  async showRawMaterials() {
    ...
    document.getElementById('content-area').innerHTML = `
      ...
      <div class="p-6"><div class="text-gray-400 text-center py-8"><i class="fas fa-spinner fa-spin text-2xl"></i></div></div>`;
    try {
      const [rmData, supData, recentData] = await Promise.all([
        this.api.get('/api/raw-materials'),
        this.api.get('/api/suppliers'),
        this.api.get('/api/raw-materials/purchases/recent?limit=500')
      ]);
      this.state.rawMaterials = rmData.items || [];
      this.state.allClients = supData.suppliers || [];
      this.state.rawPurchases = (recentData && recentData.purchases) || [];
      this._setPage('rawRecent', 1);
  ...
  renderRawMaterials() {
    ...
    area.innerHTML = `
      ...
      <div class="p-4 md:p-6 space-y-5">
        ...
        <!-- ============ RECENT ENTRIES (raw material purchases / restocks) ============ -->
        <section id="raw-recent" class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-3 border-b flex items-center justify-between flex-wrap gap-2">
            <h2 class="font-bold text-gray-800"><i class="fas fa-clock-rotate-left text-blue-500 mr-2"></i>Recent Entries</h2>
            <span class="text-xs text-gray-500">Har purchase / restock ki entry (kis material ki, kitni, kis supplier se)</span>
          </div>
          <div id="raw-recent-body">${this.renderRawRecent()}</div>
        </section>
      </div>`;
  },

  // Render the paginated raw-material "Recent Entries" list (purchases/restocks).
  renderRawRecent() {
    const all = this.state.rawPurchases || [];
    if (all.length === 0) {
      return `<p class="text-gray-500 text-center py-6"><i class="fas fa-inbox text-2xl block mb-2"></i>No purchase / restock entries yet.</p>`;
    }
    const { pageItems, totalPages, page, total } = this.paginate(all, 'rawRecent');
    const rows = pageItems.map(p => {
      const supName = p.supplier_name_resolved || p.supplier_name || '(no supplier)';
      const rem = parseFloat(p.remaining_amount) || 0;
      const supLink = p.supplier_id
        ? `<a href="#" onclick="App.openClient(${p.supplier_id}); return false;" class="text-blue-500 hover:underline">${this.escapeHtml(supName)}</a>`
        : this.escapeHtml(supName);
      return `<tr>
        <td class="text-gray-600">${this.escapeHtml(p.entry_date || '')}</td>
        <td class="font-medium">${this.escapeHtml(p.raw_name || '(deleted)')}</td>
        <td>${supLink}</td>
        <td class="text-right text-green-600 font-semibold">+${this.fmt(p.quantity)} ${this.escapeHtml(p.raw_unit || '')}</td>
        ...
        <td class="text-center whitespace-nowrap">
          <button onclick="App.editRawPurchase(${p.id})" class="btn btn-secondary btn-sm" title="Edit entry"><i class="fas fa-edit"></i></button>
          <button onclick="App.deleteRawPurchase(${p.id}, ${p.raw_material_id})" class="text-red-500 hover:text-red-700 ml-1" title="Delete entry"><i class="fas fa-trash text-sm"></i></button>
        </td>
      </tr>`;
    }).join('');
    return `
      <div class="overflow-x-auto"><table class="ledger-table text-sm">
        <thead><tr>
          <th style="width:110px;">Date</th><th>Material</th><th>Supplier</th>
          <th style="width:130px;" class="text-right">Qty</th><th style="width:100px;" class="text-right">Rate</th>
          <th style="width:120px;" class="text-right">Total</th><th style="width:120px;" class="text-right">Remaining</th>
          <th style="width:120px;" class="text-center">Action</th>
        </tr></thead><tbody>${rows}</tbody></table></div>
      ${this.renderPager('rawRecent', page, totalPages, total, 'refreshRawRecent')}`;
  },

  refreshRawRecent() {
    const el = document.getElementById('raw-recent-body');
    if (el) el.innerHTML = this.renderRawRecent();
    try { document.getElementById('raw-recent').scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch (e) {}
  },

  // Edit a raw-material purchase/restock entry from the Recent Entries list.
  async editRawPurchase(pid) {
    await this.loadBankAccounts();
    const p = (this.state.rawPurchases || []).find(x => x.id === pid);
    if (!p) return;
    const suppliers = this.state.allClients || [];
    const today = new Date().toISOString().slice(0, 10);
    this.openModal(`
      <h2 class="text-xl font-bold mb-3"><i class="fas fa-edit text-blue-500 mr-2"></i>Edit Purchase / Restock</h2>
      <p class="text-sm text-gray-500 mb-3">Material: <strong>${this.escapeHtml(p.raw_name || '')}</strong></p>
      <form id="raw-purchase-edit-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div><label class="block text-sm font-medium mb-1">Date</label>
          <input id="rpe-date" type="date" class="input-field" value="${p.entry_date || today}"></div>
        <div><label class="block text-sm font-medium mb-1">Supplier (optional)</label>
          <select id="rpe-supplier" class="input-field">
            <option value="">-- No supplier --</option>
            ${suppliers.map(s => `<option value="${s.id}" ${String(s.id) === String(p.supplier_id) ? 'selected' : ''}>${this.escapeHtml(s.name)}</option>`).join('')}
          </select></div>
        <div><label class="block text-sm font-medium mb-1">Quantity</label>
          <input id="rpe-qty" type="number" step="any" min="0" class="input-field" value="${parseFloat(p.quantity) || 0}" oninput="App._rpeCalc()"></div>
        <div><label class="block text-sm font-medium mb-1">Rate (PKR)</label>
          <input id="rpe-rate" type="number" step="any" min="0" class="input-field" value="${parseFloat(p.rate) || 0}" oninput="App._rpeCalc()"></div>
        <div><label class="block text-sm font-medium mb-1">Paid Amount (PKR)</label>
          <input id="rpe-paid" type="number" step="any" min="0" class="input-field" value="${parseFloat(p.paid_amount) || 0}" oninput="App._rpeCalc()"></div>
        <div><label class="block text-sm font-medium mb-1">Total / Remaining</label>
          <div id="rpe-preview" class="input-field bg-gray-50 flex items-center text-sm"></div></div>
        <div class="md:col-span-2">${this.bankSelectHtml('rpe-bank', p.bank_account_id || '', true, 'Pay from Bank/Cash')}
          <p class="text-xs text-red-600 mt-1"><i class="fas fa-money-bill-transfer mr-1"></i>The <strong>Paid Amount</strong> is subtracted from the selected account.</p></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Note</label>
          <input id="rpe-notes" type="text" class="input-field" value="${this.escapeAttr(p.notes || '')}"></div>
        <div class="md:col-span-2 flex gap-2 justify-end pt-3 border-t">
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save</button>
        </div>
    ...
    this._rpeCalc();
    document.getElementById('raw-purchase-edit-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const supEl = document.getElementById('rpe-supplier');
      const payload = {
        entry_date: document.getElementById('rpe-date').value,
        supplier_id: supEl.value ? parseInt(supEl.value) : null,
        supplier_name: supEl.value ? supEl.options[supEl.selectedIndex].text : '',
        quantity: parseFloat(document.getElementById('rpe-qty').value) || 0,
        rate: parseFloat(document.getElementById('rpe-rate').value) || 0,
        paid_amount: parseFloat(document.getElementById('rpe-paid').value) || 0,
        notes: document.getElementById('rpe-notes').value,
        bank_account_id: (document.getElementById('rpe-bank')?.value || null)
      };
      try {
        await this.api.put(`/api/raw-material-purchases/${pid}`, payload);
        this.closeModal();
  ...
  _rpeCalc() {
    const q = parseFloat((document.getElementById('rpe-qty') || {}).value) || 0;
    const r = parseFloat((document.getElementById('rpe-rate') || {}).value) || 0;
    const paid = parseFloat((document.getElementById('rpe-paid') || {}).value) || 0;
    const total = q * r;
    const rem = Math.max(0, total - paid);
    const el = document.getElementById('rpe-preview');
    if (el) el.innerHTML = `Total: <strong class="ml-1">PKR ${this.fmt(total)}</strong> · Remaining: <strong class="ml-1 ${rem > 0 ? 'text-red-600' : 'text-green-600'}">PKR ${this.fmt(rem)}</strong>`;
  },
  ...
  async showPaySupplier(purchaseId, rawId) {
    await this.loadBankAccounts();
    this.openModal(`
      ...
      <form id="pay-sup-form" class="space-y-3">
        ...
        <div>${this.bankSelectHtml('ps-bank', '', true, 'Pay from Bank/Cash')}
          <p class="text-xs text-red-600 mt-1"><i class="fas fa-money-bill-transfer mr-1"></i>This amount is subtracted from the selected account.</p></div>
        <div class="flex gap-2 justify-end pt-2 border-t">
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
    ...
    document.getElementById('pay-sup-form').addEventListener('submit', async (e) => {
      ...
      try {
        await this.api.post(`/api/raw-material-purchases/${purchaseId}/pay`, { amount, bank_account_id: (document.getElementById('ps-bank')?.value || null) });
        this.closeModal();
        await this.showRawMaterials();
  ...
  async deleteRawPurchase(purchaseId, rawId) {
    ...
    // Was the detail modal open when delete was clicked? (If deleted from the
    // Recent Entries list there is no modal, so we should NOT reopen the detail.)
    const detailWasOpen = !!document.querySelector('#modal-overlay .fa-cubes');
    try {
      await this.api.delete(`/api/raw-material-purchases/${purchaseId}`);
      await this.showRawMaterials();
      if (detailWasOpen && rawId) this.showRawDetail(rawId);
      this.toast('Batch removed', 'success');
    } catch (e) { this.toast('Failed', 'error'); }
  },

  async showRawEditor(id = null) {
    await this.loadBankAccounts();
      ...
      <form id="raw-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div class="md:col-span-2 p-3 bg-green-50 border border-green-200 rounded-lg">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div class="md:col-span-2">${this.bankSelectHtml('r-bank', '', true, 'Pay from Bank/Cash')}
  ...
  // Paginated Products-Manufacturing production log (assemble / paint / pack).
  renderProductProductionLog() {
    const logs = this.state.productProductionLogs || [];
    const stageLabel = (s) => s === 'assemble' ? 'Assemble' : s === 'paint' ? 'Paint' : 'Pack';
    const stageBadge = (s) => {
      const map = { assemble: 'bg-amber-100 text-amber-700 border-amber-200', paint: 'bg-indigo-100 text-indigo-700 border-indigo-200', pack: 'bg-green-100 text-green-700 border-green-200' };
    ...
    if (logs.length === 0) {
      return `<div class="overflow-x-auto"><table class="ledger-table"><tbody>
        <tr><td class="text-center py-8 text-gray-500"><i class="fas fa-inbox text-3xl mb-2 block"></i>No production logged yet. Use <strong>Log Production</strong> to record assemble / paint / pack work.</td></tr>
        </tbody></table></div>`;
    }
    const { pageItems, totalPages, page, total, start } = this.paginate(logs, 'prodLog');
    const rows = pageItems.map((l, i) => `<tr>
      <td>${start + i + 1}</td>
      <td>${l.entry_date}</td>
      ...
      <td>
        <button onclick="App.showProductProductionEditor(${l.id})" class="btn btn-secondary btn-sm" title="Edit"><i class="fas fa-edit"></i></button>
        <button onclick="App.deleteProductProduction(${l.id})" class="text-red-500 ml-1" title="Delete"><i class="fas fa-trash text-sm"></i></button>
      </td>
    </tr>`).join('');
    return `
      <div class="overflow-x-auto"><table class="ledger-table">
      ...
      ${this.renderPager('prodLog', page, totalPages, total, 'refreshProductProductionLog')}`;
  },
  ...
  // is added to that worker's profile + weekly (Fri-Thu) total.
      ...
      <div class="p-4 md:p-6 space-y-5">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-3 border-b flex items-center justify-between">
            ...
            <span class="text-xs text-gray-500">Worker per-piece production entries</span>
          </div>
          <div id="comp-log-body">${this.renderComponentProductionLog()}</div>
        </div>
        ...
        <div class="bg-teal-50 border border-teal-200 rounded-xl p-4 text-sm text-teal-900">
          ...
          System component stock barhaega, raw material kam karega (agar recipe linked hai), scrap record karega, aur worker ki profile + weekly (Friday→Thursday) total mein per-piece payout add karega.
        </div>
      </div>`;
  },

  // Paginated Components production log.
  renderComponentProductionLog() {
    const logs = this.state.productionLogs || [];
    if (logs.length === 0) {
      return `<div class="overflow-x-auto"><table class="ledger-table"><tbody>
        <tr><td class="text-center py-8 text-gray-500"><i class="fas fa-inbox text-3xl mb-2 block"></i>No production logged yet.</td></tr>
        </tbody></table></div>`;
    }
    const { pageItems, totalPages, page, total, start } = this.paginate(logs, 'compLog');
    const rows = pageItems.map((l, i) => `<tr>
      <td>${start + i + 1}</td>
      <td>${l.entry_date}</td>
      <td>${l.employee_name ? this.escapeHtml(l.employee_name) : '<span class="text-gray-400">—</span>'}</td>
      ...
      <td>
        <button onclick="App.showProductionEditor(${l.id})" class="btn btn-secondary btn-sm" title="Edit"><i class="fas fa-edit"></i></button>
        <button onclick="App.deleteProduction(${l.id})" class="text-red-500 ml-1" title="Delete"><i class="fas fa-trash text-sm"></i></button>
      </td>
    </tr>`).join('');
    return `
      <div class="overflow-x-auto"><table class="ledger-table">
      ...
      ${this.renderPager('compLog', page, totalPages, total, 'refreshComponentProductionLog')}`;
  },

  refreshComponentProductionLog() {
    const el = document.getElementById('comp-log-body');
    if (el) el.innerHTML = this.renderComponentProductionLog();
  },
    ...
    this.openModal(`
      ...
      <form id="prod-log-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div><label class="block text-sm font-medium mb-1">Scrap / Waste (raw units)</label>
          <input id="pl-scrap" type="number" step="any" class="input-field" value="${log ? (log.scrap_qty||0) : 0}" placeholder="0" oninput="App._onProdRecalc()"></div>
  ...
  _onProdRecalc() {
    ...
    const qty = parseFloat(document.getElementById('pl-qty')?.value) || 0;
    const rate = parseFloat(document.getElementById('pl-rate')?.value) || 0;
    // BUG5 FIX: scrap/reject pieces are NOT paid — payout = (qty - scrap) × rate
    const scrap = parseFloat(document.getElementById('pl-scrap')?.value) || 0;
    const payQty = Math.max(0, qty - scrap);
    const payout = payQty * rate;
    box.innerHTML = `
      <div class="bg-green-50 border border-green-200 rounded-lg p-3 flex items-center justify-between">
        <span class="text-sm text-gray-700"><i class="fas fa-coins text-green-600 mr-1"></i>Worker Payout (${this.fmt(payQty)} good pcs × PKR ${this.fmt(rate)}${scrap > 0 ? ` · ${this.fmt(scrap)} scrap not paid` : ''})</span>
  ...
  renderEmployeeDetail() {
    ...
    area.innerHTML = `
      ...
      <div class="p-4 md:p-6 space-y-5">
        ...
        <!-- Weekly Production Payout (Friday -> Thursday) -->
  ...
  // Weekly production payout grouped Friday -> Thursday.
  // Each Friday a new week starts; the per-piece earnings of that week are totalled.
  _renderWeeklyPayout() {
    ...
    if (weeks.length === 0) {
      return `
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-3 border-b flex items-center justify-between">
            <h2 class="font-bold text-gray-800"><i class="fas fa-calendar-week mr-2 text-teal-600"></i>Weekly Production Payout <span class="text-xs font-normal text-gray-500">(Friday → Thursday)</span></h2>
          </div>
    ...
    return `
      <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="px-4 py-3 border-b flex items-center justify-between flex-wrap gap-2">
          <div>
            <h2 class="font-bold text-gray-800"><i class="fas fa-calendar-week mr-2 text-teal-600"></i>Weekly Production Payout <span class="text-xs font-normal text-gray-500">(Friday → Thursday)</span></h2>
            <p class="text-xs text-gray-400 mt-0.5"><i class="fas fa-rotate mr-1"></i>Har Friday naya week shuru hota hai (auto-reset). Active advance / remaining balance reset nahi hota — woh carry-forward hota hai.</p>
          </div>
  ...
  async showEmployeeTxEditor(empId, txId = null) {
    await this.loadBankAccounts();
    ...
    this.openModal(`
      ...
      <form id="etx-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div class="md:col-span-2">${this.bankSelectHtml('etx-bank', tx.bank_account_id || '', true, 'Pay from Bank/Cash')}
          <p class="text-xs text-red-600 mt-1"><i class="fas fa-money-bill-transfer mr-1"></i>Paid salary / payment / advance amount is subtracted from the selected account (per-piece earnings that aren't paid out won't affect any bank).</p></div>
    ...
    document.getElementById('etx-form').addEventListener('submit', async (e) => {
      ...
      let payload = {
        ...
        entry_type: eType,
        bank_account_id: (document.getElementById('etx-bank')?.value || null)
      };
      if (eType === 'per_piece') {
  ...
  async showSideExpenseEditor(id = null, defaultFolderId = null) {
    await this.loadBankAccounts();
    ...
    this.openModal(`
      ...
      <form id="se-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div class="md:col-span-2">${this.bankSelectHtml('se-bank', it.bank_account_id || '', true, 'Pay from Bank/Cash')}
          <p class="text-xs text-red-600 mt-1"><i class="fas fa-money-bill-transfer mr-1"></i>This expense amount is subtracted from the selected account.</p></div>
    ...
    document.getElementById('se-form').addEventListener('submit', async (e) => {
      ...
      const payload = {
        ...
        bank_account_id: (document.getElementById('se-bank')?.value || null)
      };
      try {
    ...
    this.openModal(`
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        ...
        <div class="space-y-2">
          ...
          <div class="pt-2 border-t">${this.bankSelectHtml('b-bank', editing?.bank_account_id || '', true, 'Receive Paid amount into')}
            <p class="text-xs text-green-600 mt-1"><i class="fas fa-hand-holding-dollar mr-1"></i>The <strong>Paid</strong> amount is added to the selected bank/cash account.</p></div>
        </div>
      </div>
      <div class="flex gap-2 justify-end pt-2 border-t">
  ...
  async _saveBill(billId, alsoPrint) {
    ...
    const payload = {
      ...
      bank_account_id: (document.getElementById('b-bank')?.value || null),
  ...
  // Load accounts into state (used by other forms too). Returns the list.
  async loadBankAccounts(force = false) {
    if (!force && this.state.bankAccounts && this.state.bankAccounts.length) return this.state.bankAccounts;
    try {
      const data = await this.api.get('/api/bank-accounts');
      this.state.bankAccounts = data.accounts || [];
    } catch (e) { this.state.bankAccounts = []; }
    return this.state.bankAccounts;
  },

  // Reusable <select> of bank accounts for any payment form.
  // selectedId: currently chosen account; allowNone: show a "— No account —" option.
  bankSelectHtml(selectId, selectedId = '', allowNone = true, label = 'Bank / Cash Account') {
    const accs = this.state.bankAccounts || [];
    const opts = accs.map(a => `<option value="${a.id}" ${String(selectedId) === String(a.id) ? 'selected' : ''}>${this.escapeHtml(a.name)} (Bal: PKR ${this.fmt(a.current_balance)})</option>`).join('');
    const none = allowNone ? `<option value="">— No account (not banked) —</option>` : '';
    const hint = accs.length === 0 ? `<p class="text-xs text-orange-600 mt-1"><i class="fas fa-info-circle mr-1"></i>No bank accounts yet — add one in Banking / Payments.</p>` : '';
    return `<label class="block text-sm font-medium mb-1">${label}</label>
      <select id="${selectId}" class="input-field">${none}${opts}</select>${hint}`;
  },

  bankCategoryMeta(cat) {
    const map = {
      customer_payment: { label: 'Customer Payment', icon: 'fa-hand-holding-dollar', color: '#10b981' },
      supplier_payment: { label: 'Supplier Payment', icon: 'fa-truck', color: '#ef4444' },
      bill: { label: 'Bill Payment', icon: 'fa-file-invoice', color: '#10b981' },
      salary: { label: 'Salary', icon: 'fa-money-check-dollar', color: '#ef4444' },
      employee: { label: 'Employee', icon: 'fa-user-tie', color: '#f59e0b' },
      expense: { label: 'Side Expense', icon: 'fa-money-bill-wave', color: '#ef4444' },
      raw_material: { label: 'Raw Material', icon: 'fa-cubes', color: '#ef4444' },
      transfer: { label: 'Transfer', icon: 'fa-right-left', color: '#6366f1' },
      opening: { label: 'Opening', icon: 'fa-flag', color: '#6b7280' },
      adjustment: { label: 'Adjustment', icon: 'fa-sliders', color: '#6b7280' },
      general: { label: 'General', icon: 'fa-circle-dot', color: '#3b82f6' }
    };
    return map[cat] || map.general;
  },

  acctTypeMeta(t) {
    if (t === 'cash') return { icon: 'fa-wallet', label: 'Cash' };
    if (t === 'wallet') return { icon: 'fa-mobile-screen', label: 'Wallet' };
    return { icon: 'fa-building-columns', label: 'Bank' };
  },

  // ---------- Main Banking page (accounts overview) ----------
  async showBanking() {
    this.state.view = 'banking';
    this.state.currentBankAccountId = null;
    this.setActiveNav('banking');
    const area = document.getElementById('content-area');
    area.innerHTML = `<div class="p-6 text-center text-gray-400"><i class="fas fa-spinner fa-spin text-2xl"></i></div>`;
    let data;
    try { data = await this.api.get('/api/bank-accounts'); }
    catch (e) { area.innerHTML = `<div class="p-6 text-red-500">Failed to load banking data.</div>`; return; }
    this.state.bankAccounts = data.accounts || [];
    const accs = data.accounts || [];
    const g = data.grand || { total_balance: 0, total_in: 0, total_out: 0, count: 0 };

    area.innerHTML = `
      <div class="page-header">
        <div>
          <h1 class="page-title"><i class="fas fa-building-columns text-blue-600"></i> Banking / Payments</h1>
          <p class="page-subtitle">${accs.length} account(s) · Total balance across all accounts</p>
        </div>
        <div class="flex gap-2 flex-wrap">
          <button onclick="App.showBankTxnEditor('in')" class="btn btn-success"><i class="fas fa-arrow-down"></i> Money In</button>
          <button onclick="App.showBankTxnEditor('out')" class="btn btn-danger"><i class="fas fa-arrow-up"></i> Money Out</button>
          <button onclick="App.showBankTransferModal()" class="btn btn-secondary"><i class="fas fa-right-left"></i> Transfer</button>
          <button onclick="App.showBankAccountEditor()" class="btn btn-primary"><i class="fas fa-plus"></i> Add Account</button>
        </div>
      </div>
      <div class="p-4 md:p-6 space-y-5">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-scale-balanced mr-1"></i>Total Balance</p><p class="text-xl font-bold" style="color:#2563eb">PKR ${this.fmt(g.total_balance)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-arrow-down text-green-500 mr-1"></i>Total Received</p><p class="text-xl font-bold text-green-600">PKR ${this.fmt(g.total_in)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-arrow-up text-red-500 mr-1"></i>Total Paid</p><p class="text-xl font-bold text-red-600">PKR ${this.fmt(g.total_out)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500"><i class="fas fa-building-columns text-gray-500 mr-1"></i>Accounts</p><p class="text-xl font-bold text-gray-700">${g.count}</p></div>
        </div>

        ${accs.length === 0 ? `
          <div class="bg-white rounded-xl shadow-sm p-10 text-center text-gray-500">
            <i class="fas fa-building-columns text-4xl mb-3 block text-gray-300"></i>
            <p class="mb-3">Koi bank account nahi hai. Apna pehla account (Bank ya Cash) add karein.</p>
            <button onclick="App.showBankAccountEditor()" class="btn btn-primary"><i class="fas fa-plus"></i> Add Account</button>
          </div>` : `
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ${accs.map(a => {
              const tm = this.acctTypeMeta(a.account_type);
              return `
              <div class="bank-card" onclick="App.showBankAccount(${a.id})" style="cursor:pointer; border-top:4px solid ${a.color || '#2563eb'};">
                <div class="flex items-start justify-between">
                  <div class="flex items-center gap-3">
                    <div class="bank-icon" style="background:${a.color || '#2563eb'}"><i class="fas ${a.icon || tm.icon}"></i></div>
                    <div>
                      <h3 class="font-bold text-gray-800">${this.escapeHtml(a.name)}</h3>
                      <p class="text-xs text-gray-500">${tm.label}${a.account_number ? ' · ' + this.escapeHtml(a.account_number) : ''}</p>
                    </div>
                  </div>
                  <button onclick="event.stopPropagation(); App.showBankAccountEditor(${a.id})" class="text-gray-400 hover:text-blue-500"><i class="fas fa-ellipsis-v"></i></button>
                </div>
                <div class="mt-4">
                  <p class="text-xs text-gray-500">Current Balance</p>
                  <p class="text-2xl font-extrabold" style="color:${(parseFloat(a.current_balance)||0) >= 0 ? '#0f766e' : '#dc2626'}">PKR ${this.fmt(a.current_balance)}</p>
                </div>
                <div class="mt-3 flex justify-between text-xs pt-3 border-t">
                  <span class="text-green-600"><i class="fas fa-arrow-down mr-1"></i>In: ${this.fmt(a.total_in)}</span>
                  <span class="text-red-600"><i class="fas fa-arrow-up mr-1"></i>Out: ${this.fmt(a.total_out)}</span>
                  <span class="text-gray-500">${a.txn_count} txn</span>
                </div>
              </div>`;
            }).join('')}
          </div>`}
  ...
  // ---------- Add / Edit a bank account ----------
  showBankAccountEditor(id = null) {
    const a = id ? (this.state.bankAccounts || []).find(x => x.id === id)
                 : { name:'', account_type:'bank', account_number:'', bank_name:'', branch:'', iban:'', account_title:'', opening_balance:0, color:'#2563eb', icon:'fa-building-columns', notes:'', active:1 };
    if (id && !a) return;
    const colorOptions = ['#2563eb','#0ea5e9','#10b981','#14b8a6','#f59e0b','#ef4444','#8b5cf6','#ec4899','#6b7280','#0f766e'];
    this.openModal(`
      <h2 class="text-xl font-bold mb-4"><i class="fas fa-building-columns text-blue-600 mr-2"></i>${id ? 'Edit' : 'Add'} Bank / Cash Account</h2>
      <form id="ba-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Account Name <span class="text-red-500">*</span></label>
          <input id="ba-name" class="input-field" value="${this.escapeAttr(a.name||'')}" placeholder="e.g., Meezan Bank, Cash in Hand" required></div>
        <div><label class="block text-sm font-medium mb-1">Type</label>
          <select id="ba-type" class="input-field">
            <option value="bank" ${a.account_type==='bank'?'selected':''}>Bank</option>
            <option value="cash" ${a.account_type==='cash'?'selected':''}>Cash</option>
            <option value="wallet" ${a.account_type==='wallet'?'selected':''}>Mobile Wallet (JazzCash / Easypaisa)</option>
          </select></div>
        <div><label class="block text-sm font-medium mb-1">Opening Balance</label>
          <input id="ba-opening" type="number" step="0.01" class="input-field" value="${a.opening_balance||0}"></div>
        <div><label class="block text-sm font-medium mb-1">Account Number</label>
          <input id="ba-number" class="input-field" value="${this.escapeAttr(a.account_number||'')}"></div>
        <div><label class="block text-sm font-medium mb-1">Account Title</label>
          <input id="ba-title" class="input-field" value="${this.escapeAttr(a.account_title||'')}"></div>
        <div><label class="block text-sm font-medium mb-1">Bank / Institution</label>
          <input id="ba-bank" class="input-field" value="${this.escapeAttr(a.bank_name||'')}"></div>
        <div><label class="block text-sm font-medium mb-1">Branch</label>
          <input id="ba-branch" class="input-field" value="${this.escapeAttr(a.branch||'')}"></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">IBAN (optional)</label>
          <input id="ba-iban" class="input-field" value="${this.escapeAttr(a.iban||'')}"></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Color</label>
          <div class="flex flex-wrap gap-2">
            ${colorOptions.map(c => `<label class="cursor-pointer">
              <input type="radio" name="ba-color" value="${c}" ${a.color===c?'checked':''} class="ba-color-radio hidden">
              <span class="w-9 h-9 rounded-lg border-2 flex items-center justify-center" data-bacolor="${c}" style="background:${c}; border-color:${a.color===c?'#1f2937':'#e5e7eb'};">${a.color===c?'<i class="fas fa-check text-white"></i>':''}</span>
            </label>`).join('')}
          </div></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Notes</label>
          <input id="ba-notes" class="input-field" value="${this.escapeAttr(a.notes||'')}"></div>
        ${id ? `<div class="md:col-span-2 text-xs text-gray-500 bg-gray-50 p-2 rounded"><i class="fas fa-info-circle mr-1"></i>Current balance = opening balance + all received − all paid. Opening balance change karne se current balance auto-recompute hoga.</div>` : ''}
        <div class="md:col-span-2 flex gap-2 justify-end pt-2 border-t">
          ${id ? `<button type="button" class="btn btn-danger mr-auto" onclick="App.deleteBankAccount(${id})"><i class="fas fa-trash"></i> Delete</button>` : ''}
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Save</button>
        </div>
      </form>`, 'modal-lg');
    document.querySelectorAll('.ba-color-radio').forEach(r => r.addEventListener('change', () => {
      document.querySelectorAll('[data-bacolor]').forEach(b => { b.style.borderColor='#e5e7eb'; b.innerHTML=''; });
      const box = document.querySelector(`[data-bacolor="${r.value}"]`);
      if (box) { box.style.borderColor='#1f2937'; box.innerHTML='<i class="fas fa-check text-white"></i>'; }
    }));
    document.getElementById('ba-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = document.getElementById('ba-name').value.trim();
      if (!name) { this.toast('Account name required', 'error'); return; }
      const colorR = document.querySelector('.ba-color-radio:checked');
      const payload = {
        name,
        account_type: document.getElementById('ba-type').value,
        opening_balance: parseFloat(document.getElementById('ba-opening').value) || 0,
        account_number: document.getElementById('ba-number').value,
        account_title: document.getElementById('ba-title').value,
        bank_name: document.getElementById('ba-bank').value,
        branch: document.getElementById('ba-branch').value,
        iban: document.getElementById('ba-iban').value,
        notes: document.getElementById('ba-notes').value,
        color: colorR ? colorR.value : (a.color || '#2563eb'),
        icon: this.acctTypeMeta(document.getElementById('ba-type').value).icon,
        active: 1
      };
      try {
        if (id) await this.api.put(`/api/bank-accounts/${id}`, payload);
        else await this.api.post('/api/bank-accounts', payload);
        this.closeModal();
        await this.loadBankAccounts(true);
        if (this.state.currentBankAccountId) await this.showBankAccount(this.state.currentBankAccountId);
        else await this.showBanking();
        this.toast('Account saved', 'success');
      } catch (err) { this.toast(err.message || 'Failed to save', 'error'); }
    });
  },

  async deleteBankAccount(id) {
    if (!confirm('Is account ko delete karein? Agar is ke transactions hain to woh bhi delete honge aur linked records unlink ho jayenge.')) return;
    try {
      // Try normal delete first; if it has txns, backend returns error → offer force
      const res = await fetch(`/api/bank-accounts/${id}`, { method: 'DELETE', credentials: 'include' });
      const data = await res.json();
      if (!res.ok) {
        if (data.txn_count && confirm(`${data.error}\n\nForce delete all ${data.txn_count} transaction(s)?`)) {
          await fetch(`/api/bank-accounts/${id}?force=1`, { method: 'DELETE', credentials: 'include' });
        } else { this.toast(data.error || 'Delete failed', 'error'); return; }
      }
      this.closeModal();
      await this.loadBankAccounts(true);
  ...
  // ---------- Single account detail + statement ----------
  async showBankAccount(id) {
    this.state.view = 'banking';
    this.state.currentBankAccountId = id;
    this.setActiveNav('banking');
    const area = document.getElementById('content-area');
    area.innerHTML = `<div class="p-6 text-center text-gray-400"><i class="fas fa-spinner fa-spin text-2xl"></i></div>`;
    const from = this._bankFilter?.from || '';
    const to = this._bankFilter?.to || '';
    let qs = '';
    if (from) qs += `${qs?'&':'?'}from=${from}`;
    if (to) qs += `${qs?'&':'?'}to=${to}`;
    let data;
    try { data = await this.api.get(`/api/bank-accounts/${id}${qs}`); }
    catch (e) { area.innerHTML = `<div class="p-6 text-red-500">Failed to load account.</div>`; return; }
    const a = data.account;
    this.state.currentBankAccount = a;
    const txns = data.transactions || [];
    const tm = this.acctTypeMeta(a.account_type);
    const periodIn = txns.filter(t => t.direction==='in').reduce((s,t)=>s+(parseFloat(t.amount)||0),0);
    const periodOut = txns.filter(t => t.direction==='out').reduce((s,t)=>s+(parseFloat(t.amount)||0),0);
    ...
    area.innerHTML = `
      <div class="page-header">
        <div>
          <h1 class="page-title">
            <button onclick="App.showBanking()" class="text-gray-500 hover:text-gray-800 mr-2" title="Back"><i class="fas fa-arrow-left"></i></button>
            <i class="fas ${a.icon || tm.icon}" style="color:${a.color||'#2563eb'}"></i> ${this.escapeHtml(a.name)}
          </h1>
          <p class="page-subtitle">${tm.label}${a.account_number ? ' · ' + this.escapeHtml(a.account_number) : ''}${a.bank_name ? ' · ' + this.escapeHtml(a.bank_name) : ''}</p>
        </div>
        <div class="flex gap-2 flex-wrap">
          <button onclick="App.showBankTxnEditor('in', ${a.id})" class="btn btn-success"><i class="fas fa-arrow-down"></i> Money In</button>
          <button onclick="App.showBankTxnEditor('out', ${a.id})" class="btn btn-danger"><i class="fas fa-arrow-up"></i> Money Out</button>
          <button onclick="App.showBankStatementModal(${a.id})" class="btn btn-primary"><i class="fas fa-file-pdf"></i> Statement PDF</button>
          <button onclick="App.showBankAccountEditor(${a.id})" class="btn btn-secondary"><i class="fas fa-edit"></i> Edit</button>
        </div>
      </div>
      <div class="p-4 md:p-6 space-y-5">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="stat-card"><p class="text-xs text-gray-500">Current Balance</p><p class="text-xl font-bold" style="color:${(parseFloat(a.current_balance)||0)>=0?'#0f766e':'#dc2626'}">PKR ${this.fmt(a.current_balance)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500">Opening Balance</p><p class="text-xl font-bold text-gray-600">PKR ${this.fmt(a.opening_balance)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500">Received (period)</p><p class="text-xl font-bold text-green-600">PKR ${this.fmt(periodIn)}</p></div>
          <div class="stat-card"><p class="text-xs text-gray-500">Paid (period)</p><p class="text-xl font-bold text-red-600">PKR ${this.fmt(periodOut)}</p></div>
        </div>

        <div class="bg-white rounded-xl shadow-sm p-3 flex flex-wrap items-end gap-3">
          <div><label class="block text-xs text-gray-500 mb-1">From</label><input id="bank-from" type="date" class="input-field" style="width:auto" value="${from}"></div>
          <div><label class="block text-xs text-gray-500 mb-1">To</label><input id="bank-to" type="date" class="input-field" style="width:auto" value="${to}"></div>
          <button onclick="App.applyBankFilter(${a.id})" class="btn btn-secondary btn-sm"><i class="fas fa-filter"></i> Filter</button>
          ${(from||to) ? `<button onclick="App.clearBankFilter(${a.id})" class="btn btn-secondary btn-sm"><i class="fas fa-times"></i> Clear</button>` : ''}
        </div>

        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
          <div class="overflow-x-auto"><table class="ledger-table">
            <thead><tr>
              <th style="width:40px;">#</th><th style="width:110px;">Date</th>
              <th>Description</th><th>Category</th><th>Party</th>
              <th class="text-right">In</th><th class="text-right">Out</th>
              <th class="text-right">Balance</th><th style="width:90px;">Action</th>
            </tr></thead><tbody>
              ${rows.length === 0 ? `<tr><td colspan="9" class="text-center py-8 text-gray-500"><i class="fas fa-receipt text-3xl mb-2 block"></i>No transactions yet.</td></tr>` :
                rows.map((t, i) => {
                  const cm = this.bankCategoryMeta(t.category);
                  const isManual = t.source === 'manual';
                  return `<tr>
                    <td>${rows.length - i}</td>
                    <td>${t.entry_date}</td>
                    <td>${this.escapeHtml(t.description||'')}${t.reference?`<span class="text-xs text-gray-400 block">Ref: ${this.escapeHtml(t.reference)}</span>`:''}</td>
                    <td><span class="text-xs px-2 py-0.5 rounded-full" style="background:${cm.color}22; color:${cm.color}"><i class="fas ${cm.icon} mr-1"></i>${cm.label}</span></td>
                    <td>${this.escapeHtml(t.party_name||'')}</td>
                    <td class="text-right font-bold text-green-600">${t.direction==='in'?'PKR '+this.fmt(t.amount):''}</td>
                    <td class="text-right font-bold text-red-600">${t.direction==='out'?'PKR '+this.fmt(t.amount):''}</td>
                    <td class="text-right font-semibold">PKR ${this.fmt(t.running_balance)}</td>
                    <td>
                      ${isManual ? `<button onclick="App.showBankTxnEditor('${t.direction}', ${a.id}, ${t.id})" class="btn btn-secondary btn-sm"><i class="fas fa-edit"></i></button>
                       <button onclick="App.deleteBankTxn(${t.id}, ${a.id})" class="text-red-500 ml-1"><i class="fas fa-trash text-sm"></i></button>`
                       : `<span class="text-xs text-gray-400" title="Ye transaction ${cm.label} module se linked hai. Wahin se edit/delete karein."><i class="fas fa-link"></i> linked</span>`}
                    </td>
                  </tr>`;
                }).join('')}
            </tbody></table></div>
  ...
  applyBankFilter(id) {
    this._bankFilter = { from: document.getElementById('bank-from').value, to: document.getElementById('bank-to').value };
    this.showBankAccount(id);
  },
  clearBankFilter(id) { this._bankFilter = { from:'', to:'' }; this.showBankAccount(id); },

  // ---------- Manual money in / out ----------
  async showBankTxnEditor(direction = 'in', accountId = null, txnId = null) {
    await this.loadBankAccounts(true);
    if ((this.state.bankAccounts||[]).length === 0) {
      this.toast('Pehle ek bank/cash account add karein', 'error');
      return this.showBankAccountEditor();
    }
    let t = { entry_date: new Date().toISOString().slice(0,10), account_id: accountId || (this.state.bankAccounts[0]||{}).id, direction, amount: 0, category: direction==='in'?'general':'general', payment_method:'', reference:'', party_name:'', description:'' };
    if (txnId) {
      // find it from the current account view rows if available
      try {
        const d = await this.api.get(`/api/bank-transactions?account_id=${accountId}&limit=2000`);
        const found = (d.transactions||[]).find(x => x.id === txnId);
        if (found) t = found;
      } catch (e) {}
    }
    const inCats = [['general','General Income'],['customer_payment','Customer Payment'],['adjustment','Adjustment']];
    const outCats = [['general','General Expense'],['supplier_payment','Supplier Payment'],['salary','Salary'],['employee','Employee'],['expense','Expense'],['raw_material','Raw Material'],['adjustment','Adjustment']];
    const cats = direction==='in' ? inCats : outCats;
    const methods = ['','cash','online','cheque','card','other'];
    this.openModal(`
      <h2 class="text-xl font-bold mb-4"><i class="fas ${direction==='in'?'fa-arrow-down text-green-600':'fa-arrow-up text-red-600'} mr-2"></i>${txnId?'Edit':'Record'} Money ${direction==='in'?'In (Received)':'Out (Paid)'}</h2>
      <form id="bt-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div><label class="block text-sm font-medium mb-1">Date</label><input id="bt-date" type="date" class="input-field" value="${t.entry_date}"></div>
        <div>${this.bankSelectHtml('bt-account', t.account_id, false)}</div>
        <div><label class="block text-sm font-medium mb-1">Amount <span class="text-red-500">*</span></label><input id="bt-amount" type="number" step="0.01" class="input-field" value="${t.amount||''}" required></div>
        <div><label class="block text-sm font-medium mb-1">Category</label><select id="bt-category" class="input-field">${cats.map(c=>`<option value="${c[0]}" ${t.category===c[0]?'selected':''}>${c[1]}</option>`).join('')}</select></div>
        <div><label class="block text-sm font-medium mb-1">Payment Method</label><select id="bt-method" class="input-field">${methods.map(m=>`<option value="${m}" ${t.payment_method===m?'selected':''}>${m?m.charAt(0).toUpperCase()+m.slice(1):'— none —'}</option>`).join('')}</select></div>
        <div><label class="block text-sm font-medium mb-1">Reference / Cheque No</label><input id="bt-ref" class="input-field" value="${this.escapeAttr(t.reference||'')}"></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">${direction==='in'?'Received From':'Paid To'}</label><input id="bt-party" class="input-field" value="${this.escapeAttr(t.party_name||'')}" placeholder="Name"></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Description</label><input id="bt-desc" class="input-field" value="${this.escapeAttr(t.description||'')}"></div>
        <div class="md:col-span-2 flex gap-2 justify-end pt-2 border-t">
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button type="submit" class="btn ${direction==='in'?'btn-success':'btn-danger'}"><i class="fas fa-save"></i> Save</button>
        </div>
      </form>`, 'modal-lg');
    document.getElementById('bt-form').addEventListener('submit', async (e) => {
      ...
      if (amount <= 0) { this.toast('Amount must be > 0', 'error'); return; }
      const payload = {
        account_id: parseInt(document.getElementById('bt-account').value),
        entry_date: document.getElementById('bt-date').value,
        direction,
        amount,
        category: document.getElementById('bt-category').value,
        payment_method: document.getElementById('bt-method').value,
        reference: document.getElementById('bt-ref').value,
        party_name: document.getElementById('bt-party').value,
        description: document.getElementById('bt-desc').value
      };
      try {
        if (txnId) await this.api.put(`/api/bank-transactions/${txnId}`, payload);
        else await this.api.post('/api/bank-transactions', payload);
        this.closeModal();
        await this.loadBankAccounts(true);
        if (this.state.currentBankAccountId) await this.showBankAccount(this.state.currentBankAccountId);
        else await this.showBanking();
        this.toast('Transaction saved', 'success');
      } catch (err) { this.toast(err.message || 'Failed', 'error'); }
    });
  },

  async deleteBankTxn(id, accountId) {
    if (!confirm('Delete this transaction? Balance auto-adjust hoga.')) return;
    try {
      await this.api.delete(`/api/bank-transactions/${id}`);
      await this.loadBankAccounts(true);
      await this.showBankAccount(accountId);
      this.toast('Transaction deleted', 'success');
    } catch (e) { this.toast(e.message || 'Failed', 'error'); }
  },

  // ---------- Transfer between accounts ----------
  async showBankTransferModal() {
    await this.loadBankAccounts(true);
    const accs = this.state.bankAccounts || [];
    if (accs.length < 2) { this.toast('Transfer ke liye kam se kam 2 accounts chahiye', 'error'); return; }
    const opt = (sel) => accs.map(a => `<option value="${a.id}">${this.escapeHtml(a.name)} (PKR ${this.fmt(a.current_balance)})</option>`).join('');
    this.openModal(`
      <h2 class="text-xl font-bold mb-4"><i class="fas fa-right-left text-indigo-600 mr-2"></i>Transfer Between Accounts</h2>
      <form id="tf-form" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div><label class="block text-sm font-medium mb-1">From Account</label><select id="tf-from" class="input-field">${opt()}</select></div>
        <div><label class="block text-sm font-medium mb-1">To Account</label><select id="tf-to" class="input-field">${opt()}</select></div>
        <div><label class="block text-sm font-medium mb-1">Amount <span class="text-red-500">*</span></label><input id="tf-amount" type="number" step="0.01" class="input-field" required></div>
        <div><label class="block text-sm font-medium mb-1">Date</label><input id="tf-date" type="date" class="input-field" value="${new Date().toISOString().slice(0,10)}"></div>
        <div class="md:col-span-2"><label class="block text-sm font-medium mb-1">Note</label><input id="tf-desc" class="input-field" placeholder="Optional"></div>
        <div class="md:col-span-2 flex gap-2 justify-end pt-2 border-t">
          <button type="button" class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button type="submit" class="btn btn-primary"><i class="fas fa-right-left"></i> Transfer</button>
        </div>
      </form>`);
    // default to-account = second account
    document.getElementById('tf-to').selectedIndex = 1;
    document.getElementById('tf-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const payload = {
        from_account_id: parseInt(document.getElementById('tf-from').value),
        to_account_id: parseInt(document.getElementById('tf-to').value),
      ...
      try {
        await this.api.post('/api/bank-transfers', payload);
        this.closeModal();
        await this.loadBankAccounts(true);
        if (this.state.currentBankAccountId) await this.showBankAccount(this.state.currentBankAccountId);
        else await this.showBanking();
        this.toast('Transfer complete', 'success');
      } catch (err) { this.toast(err.message || 'Transfer failed', 'error'); }
    });
  },

  // ---------- Bank statement PDF ----------
  showBankStatementModal(id) {
    this.openModal(`
      <h2 class="text-xl font-bold mb-3"><i class="fas fa-file-pdf text-red-500 mr-2"></i>Bank Statement PDF</h2>
      <div class="space-y-3">
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm font-medium mb-1">From (optional)</label><input id="stmt-from" type="date" class="input-field"></div>
          <div><label class="block text-sm font-medium mb-1">To (optional)</label><input id="stmt-to" type="date" class="input-field"></div>
        </div>
        <p class="text-xs text-gray-500">Khaali chhorne par poora statement (start se aaj tak) banega.</p>
        <div class="flex gap-2 justify-end pt-2 border-t">
          <button class="btn btn-secondary" onclick="App.closeModal()">Cancel</button>
          <button class="btn btn-primary" onclick="App.downloadBankStatement(${id})"><i class="fas fa-download"></i> Download PDF</button>
        </div>
      </div>`);
  },

  async downloadBankStatement(id) {
    const from = document.getElementById('stmt-from')?.value || '';
    const to = document.getElementById('stmt-to')?.value || '';
    const jsPDFCtor = (window.jspdf && window.jspdf.jsPDF) || window.jsPDF;
    if (!jsPDFCtor) { this.toast('PDF library load nahi hui — page refresh karein', 'error'); return; }
    let qs = '';
    if (from) qs += `${qs?'&':'?'}from=${from}`;
    if (to) qs += `${qs?'&':'?'}to=${to}`;
    let data;
    try { data = await this.api.get(`/api/bank-accounts/${id}${qs}`); }
    catch (e) { this.toast('Failed to load statement', 'error'); return; }
    const a = data.account;
    const txns = data.transactions || [];
    const b = this.state.branding || {};
    const openingForPeriod = data.period_opening != null ? data.period_opening : (a.opening_balance||0);
    ...
    pdf.text(b.company_name || 'Two Star Industries', margin, 13);
    pdf.setFont('helvetica','normal'); pdf.setFontSize(10);
    pdf.text('Bank Account Statement', margin, 20);
    if (b.bill_phone) pdf.text('Ph: ' + b.bill_phone, margin, 26);
    pdf.setFontSize(9);
    pdf.text(new Date().toLocaleString('en-PK'), pageW - margin, 20, { align: 'right' });
    y = 38;

    pdf.setTextColor(30,30,30);
    pdf.setFont('helvetica','bold'); pdf.setFontSize(13);
    pdf.text(a.name, margin, y); y += 6;
    pdf.setFont('helvetica','normal'); pdf.setFontSize(9);
    const meta = [];
    if (a.account_number) meta.push('A/C: ' + a.account_number);
    if (a.bank_name) meta.push(a.bank_name);
    if (a.branch) meta.push('Branch: ' + a.branch);
    if (a.iban) meta.push('IBAN: ' + a.iban);
    if (meta.length) { pdf.text(meta.join('   |   '), margin, y); y += 5; }
    const period = (from || to) ? `Period: ${from||'start'} to ${to||'today'}` : 'Period: All time';
    pdf.text(period, margin, y); y += 7;

    // Summary line
    const totIn = txns.filter(t=>t.direction==='in').reduce((s,t)=>s+(parseFloat(t.amount)||0),0);
    const totOut = txns.filter(t=>t.direction==='out').reduce((s,t)=>s+(parseFloat(t.amount)||0),0);
    pdf.setFillColor(243,244,246); pdf.rect(margin, y-4, pageW-margin*2, 8, 'F');
    pdf.setFont('helvetica','bold'); pdf.setFontSize(9);
    pdf.text(`Opening: PKR ${this.fmt(openingForPeriod)}`, margin+2, y+1);
    pdf.setTextColor(16,122,110);
    pdf.text(`In: PKR ${this.fmt(totIn)}`, margin+62, y+1);
    pdf.setTextColor(220,38,38);
    pdf.text(`Out: PKR ${this.fmt(totOut)}`, margin+108, y+1);
    pdf.setTextColor(37,99,235);
    pdf.text(`Closing: PKR ${this.fmt(a.current_balance)}`, pageW-margin-2, y+1, { align:'right' });
    pdf.setTextColor(30,30,30);
    y += 10;

    // Table header
    const cols = [
      { t:'Date', x: margin, w: 22 },
      { t:'Description', x: margin+22, w: 62 },
      { t:'In', x: margin+84, w: 30, align:'right' },
      { t:'Out', x: margin+114, w: 30, align:'right' },
      { t:'Balance', x: margin+144, w: pageW-margin-(margin+144), align:'right' }
    ];
    const drawHead = () => {
      pdf.setFillColor(37,99,235); pdf.setTextColor(255,255,255);
      pdf.rect(margin, y-4, pageW-margin*2, 7, 'F');
      pdf.setFont('helvetica','bold'); pdf.setFontSize(8.5);
      cols.forEach(c => pdf.text(c.t, c.align==='right'? c.x+c.w-1 : c.x+1, y+0.5, { align: c.align||'left' }));
    ...
    let alt = false;
    for (const t of txns) {
      run += (t.direction==='in'?1:-1)*(parseFloat(t.amount)||0);
      if (y > 280) { pdf.addPage(); y = margin+4; drawHead(); pdf.setFontSize(8); }
      if (alt) { pdf.setFillColor(249,250,251); pdf.rect(margin, y-3.5, pageW-margin*2, 6, 'F'); }
      alt = !alt;
      const desc = (t.description || this.bankCategoryMeta(t.category).label) + (t.party_name ? ' — ' + t.party_name : '');
      pdf.text(String(t.entry_date||''), cols[0].x+1, y);
      pdf.text(pdf.splitTextToSize(desc, cols[1].w-2)[0] || '', cols[1].x+1, y);
      if (t.direction==='in') { pdf.setTextColor(16,122,110); pdf.text(this.fmt(t.amount), cols[2].x+cols[2].w-1, y, {align:'right'}); pdf.setTextColor(30,30,30); }
      if (t.direction==='out') { pdf.setTextColor(220,38,38); pdf.text(this.fmt(t.amount), cols[3].x+cols[3].w-1, y, {align:'right'}); pdf.setTextColor(30,30,30); }
      pdf.setFont('helvetica','bold'); pdf.text(this.fmt(run), cols[4].x+cols[4].w-1, y, {align:'right'}); pdf.setFont('helvetica','normal');
      y += 6;
    }
    if (txns.length === 0) { pdf.text('No transactions in this period.', margin+1, y+2); }

    // Footer
    const pages = pdf.internal.getNumberOfPages();
    for (let i=1;i<=pages;i++){ pdf.setPage(i); pdf.setFontSize(8); pdf.setTextColor(150,150,150);
      pdf.text(`${b.crm_name||'Two Star CRM'} — Page ${i}/${pages}`, pageW/2, 292, {align:'center'}); }

    const fname = `Statement_${a.name.replace(/[^a-z0-9]/gi,'_')}_${new Date().toISOString().slice(0,10)}.pdf`;
    pdf.save(fname);
    this.closeModal();
